...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        spots = Spot.objects.select_related("section", "section__parking_area").all().order_by("spot_code")
        for spot in spots:
            sc = spot.spot_code
            sec_id = spot.section.id
            
            redis_client.hset(f"spot:{sc}:meta", mapping={
                "id": spot.id,
                "section_id": sec_id,
        ...
        self.stdout.write(self.style.SUCCESS("Topology Cached! Zero-Latency MQTT Routing Active."))
        ...
        try:
            ...
            client = mqtt.Client(CallbackAPIVersion.VERSION2, client_id=unique_client_id)
        except:
        ...
        def on_connect(client, userdata, flags, rc, properties=None):
            if rc == 0:
                self.stdout.write(self.style.SUCCESS(f"CONNECTED: MQTT Router [{unique_client_id}] Active"))
        ...
        def on_message(client, userdata, msg):
            try:
                # Block retained (old ghost) messages
                if msg.retain:
                    return
                ...
                if topic_parts[3] == "status" and "spots" in payload:
                    ...
                    if (now - float(redis_client.get(device_key) or 0)) > 10.0:
                        update_device_heartbeat(
                            device_uid, payload.get("ip"), payload.get("mac"),
                            [s.get("spot") for s in payload["spots"] if "spot" in s],
                            sum(1 for s in payload["spots"] if s.get("status", "") != "OFFLINE")
                        )
                        redis_client.set(device_key, now)
                    spot_code = [f"{s.get('spot')}- {s.get('status')} " for s in payload["spots"]]
                    print(f"Time {datetime.now().strftime('%H:%M:%S')}  - {spot_code}")
                    # --- SPOT ROUTING ---
                    for spot_data in payload["spots"]:
                        ...
                        threshold = getattr(settings, f"{new_status}_STABLE_SECONDS", 0.5)
                        ...
                        if new_status != last_raw:
                            ...
                            if (now - started_at) >= float(threshold) and is_synced == "0":
                                ...
                                sec_spot_codes = redis_client.smembers(f"section:{sec_id}:spots")
                                all_section_data = []
                                ...
                                if is_grid_view:
                                    ...
                                    avail = [s for s in all_section_data if s["status"] == "AVAILABLE"][:6]
                                    display_payload.extend(avail)
                                    if len(display_payload) < 6:
                                        needed = 6 - len(display_payload)
                                        others = [s for s in all_section_data if s["status"] in ["OFFLINE", "OCCUPIED"]][:needed]
                                ...
                                final_payload = [{
                                    ...
                                    "status": s["status"] if s["status"] != "OFFLINE" else s["offline_last"]
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS("MQTT Listener completely stopped. Safe to exit."))
            sys.exit(0)
